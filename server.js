...
const commands = {
	...
	'help': (msg) => {
		let tosend = ['```xl', tokens.prefix + 'join : "Joins VC of whoever sent the message"',	tokens.prefix + 'add : "Add songs to the queue (links only)"', tokens.prefix + 'queue : "Shows the line-up of songs waiting to play"', tokens.prefix + 'play : "Play the music queue if already joined to a voice channel"', '', 'if the music bot is running => {'.toUpperCase(), tokens.prefix + 'pause : "Pauses the music playing"',	tokens.prefix + 'resume : "Resumes the music playing"', tokens.prefix + 'skip : "Skips the song playing"', tokens.prefix + 'time : "Shows the playtime of the song."', tokens.prefix + 'info : "Displays information about the song being played."',	'volume+ : "increases volume by 2%/+"',	'volume- : "decreases volume by 2%/-"',	'```'];
